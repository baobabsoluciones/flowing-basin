- `T0XY`: training a SAC agent.
- `T1XY`: same as T0, but saving the best agent on evaluation rewards (EvalCallback)
and not rollout rewards (CheckpointCallback).
- `T2XY`: same as T1, but with a more aggressive learning rate (1e-3 instead of 3e-4).
- `T3XY`: same as T1, but with a smaller replay buffer size (100_000 instead of 1_000_000).
...
- `T4XY`: this combines T2 and T3 (more aggressive learning rate and smaller replay buffer).
