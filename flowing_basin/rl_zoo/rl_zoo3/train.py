...
from flowing_basin.rl_zoo.rl_zoo3.exp_manager import ExperimentManager
from flowing_basin.rl_zoo.rl_zoo3.utils import ALGOS, StoreDict
...
def train() -> None:
    ...
    parser.add_argument(
        "--continuous-actions", action="store_true", default=False,
        help="whether actions are continuous or not"
    )
    ...
    # Register flowing-basin environment
    if args.env.startswith("RLEnvironment-"):
        env_id = args.env.split("-")[1]

        def make_rl_environment():
            rl = ReinforcementLearning(f"rl-{env_id}", verbose=3)
            return rl.create_train_env()

        register(
            id=f"RLEnvironment-{env_id}",
            entry_point=make_rl_environment,
        )
