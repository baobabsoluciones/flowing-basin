...
TOPOLOGIES = ["star", "ring", "von_neumann", "pyramid", "random"]
...
for topology, example, num_dams in product(TOPOLOGIES, INSTANCES, NUMS_DAMS):

    path_instance = f"../instances/instances_big/instance{example}_{num_dams}dams_1days.json"
    path_sol = f"../solutions/test_pso_rbo_topology/instance{example}_PSO-RBO_{num_dams}dams_1days" \
               f"_t={topology}.json"
    print(f"---- Solving instance {example} with {num_dams} dams using t={topology} ----")
