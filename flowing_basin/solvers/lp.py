...
class LPModel(Experiment): 
    ...
    def LPModel_print(self):
        ...
        T = self.instance.get_largest_impact_horizon()
        L = {
            dam_id: self.instance.get_relevant_lags_of_dam(dam_id)
            for dam_id in self.instance.get_ids_of_dams()
        }
        Tramos = {
            dam_id: list(range(len(self.instance.get_turbined_flow_obs_for_power_group(dam_id)["observed_flows"]) - 1))
            for dam_id in self.instance.get_ids_of_dams()
        }
        BreakPoints = {
            dam_id: list(range(len(self.instance.get_turbined_flow_obs_for_power_group(dam_id)["observed_flows"])))
            for dam_id in self.instance.get_ids_of_dams()
        }
        ...
        BP = {
            dam_id: self.instance.get_turbined_flow_obs_for_power_group(dam_id)["observed_flows"]
            for dam_id in self.instance.get_ids_of_dams()    
        }
        
        PotBP = {
            dam_id: self.instance.get_turbined_flow_obs_for_power_group(dam_id)["observed_powers"]
            for dam_id in self.instance.get_ids_of_dams()    
        }
        ...
        print(I, T, L, Tramos, BreakPoints, D, Qnr, QMax, BP, PotBP, V0, VMax, VMin, A, B, IniLags, TMin, VolFinal, Q0)
        ...
        for i in I:
            for t in T:
                if i == "dam1":
                    ...
                    if t == 1:
                        pl24h += qtb[(i, t)] == (IniLags[i][0] + qs[(i, 0)]) / len(
                            L[i]
                        )
                    if t >= 2:
