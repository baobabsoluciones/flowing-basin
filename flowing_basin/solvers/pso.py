...
class PSO:
    def __init__(self):
        ...
        self.river_basin = RiverBasin(instance=self.instance, paths_power_models=paths_power_models)
        ...
        self.constraints = ([0 for i in range(self.dim)],
                       [1 for i in range(self.dim)])

        self.flows = dict()

    def particle_to_flows(self, particle):
        flows = {(p, d): particle[p * len(self.dams) + self.dams.index(d)] for p in
                        range(self.no_periods) for d in self.dams}
        return flows
    ...
    def get_income(self, particle):
        ...
        for p in range(self.no_periods):
            f = {d: flows[p,d] for d in self.dams}
            self.river_basin.update(f)
            state = self.river_basin.get_state()
            print("period {}".format(p))
            income += state['price'] * (state['dam1']['power'] + state['dam2']['power']) * self.instance.get_time_step() / 3600
        return income
...
# def particle_to_flows(particle, instance, no_periods):
#     flows = {(p, d): particle[p * len(instance.dams) + instance.dams.index(d)] for p in
#                     range(no_periods) for d in instance.dams}
#     return flows
#
#
#
...
#     global no_periods
#     income = 0
#     flows = particle_to_flows(X, instance, no_periods)
#     for p in range(no_periods):
#         f = {d: flows[p,d] for d in instance.dams}
#         instance.river_basin.update(f)
#         state = instance.river_basin.get_state()
#         income += state['price'] * (state['dam1']['power'] + state['dam2']['power']) * instance.get_time_step() / 3600
#     return income
#
#
#
# def run_pso(swarm_size, dim, options, constraints):
#     # Call an instance of PSO
...
#
#     # Perform optimization
#     cost, flows = optimizer.optimize(get_income, iters=iters)
#     print("cost: {}".format(cost))
#     print(flows)
#     print("end")
#
...
#     # dams = list(self.instance.data["dams"].keys())
#     no_periods = 100  # 7 * 24 * 4
#     swarm_size = 20
#     dim = len(instance.data['dams']) * no_periods
#     iters = 1000
#     # epsilon = 1.0
#     options = {'c1': 1, 'c2': 0.5, 'w': 0.9}
#     constraints = ([0 for i in range(dim)],
#                    [1 for i in range(dim)])
#
#     run_pso(swarm_size, dim, options, constraints)
#     print("end")